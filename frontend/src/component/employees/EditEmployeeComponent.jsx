...
import { useEditEmployee, usePostEmployeeData } from '../../services/employeeServices'
...
export const EditEmployeeComponent = () => {
    ...
    var { register, handleSubmit, formState: { errors } } = useForm({
        defaultValues: async () => {
            ...
            const employee = await axios.get("http://localhost:9999/inverntoryManagment/api/v1/public/employee/employee/" + id)
            console.log("emp : ", employee);
            return {
                name: employee?.data?.data.name,
                phoneNumber: employee?.data?.data.phoneNumber,
                phoneNumber2: employee?.data?.data.phoneNumber2,
                email: employee?.data?.data.email,
                email2: employee?.data?.data.email2,
                designation: employee?.data?.data.designation,
    ...
    const mutation = useEditEmployee();
    ...
    useEffect(() => {
        ...
        if (mutation.data) {
            notifyDone("Employee edited successfully.")
                    ...
                    <section className="section">
                        <div className="card">
                            <div className="card-header">
                                <h4 className="card-title">Edit employee's details</h4>
                            </div>
